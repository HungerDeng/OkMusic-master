...
import com.gdut.dkmfromcg.playmusiclib.util.BitmapHelper;
...
public final class AlbumArtCache {
    ...
    public void fetch(final String artUrl, final FetchListener listener) {
        ...
        if (bitmap != null) {
            Logger.d(TAG, "getOrFetch: album art is in cache, using it" + artUrl);
            listener.onFetched(artUrl, bitmap[BIG_BITMAP_INDEX], bitmap[ICON_BITMAP_INDEX]);
            return;
        }
        Logger.d(TAG, "getOrFetch: starting asynctask to fetch " + artUrl);
        ...
        new AsyncTask<Void, Void, Bitmap[]>() {
            ...
            protected Bitmap[] doInBackground(Void[] objects) {
                ...
                Logger.d(TAG, "doInBackground: putting bitmap in cache. cache size=" +
                        mCache.size());
                return bitmaps;
            }
    ...
    public static abstract class FetchListener {
        ...
        public void onError(String artUrl, Exception e) {
            Logger.e(TAG, Logger.getStackTraceString(e) + "AlbumArtFetchListener: error while downloading " + artUrl);
        }
    }
}
