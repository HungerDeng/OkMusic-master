...
package com.gdut.dkmfromcg.playmusiclib.util;
...
import com.gdut.dkmfromcg.playmusiclib.model.VoiceSearchParams;
...
public class QueueHelper {
    ...
    public static List<MediaSessionCompat.QueueItem> getPlayingQueue(String mediaId,
        ...
        if (hierarchy.length != 2) {
            Logger.e(TAG, "Could not build a playing queue for this mediaId: " + mediaId);
            return null;
        }

        String categoryType = hierarchy[0];
        String categoryValue = hierarchy[1];
        Logger.d(TAG, "Creating playing queue for " + categoryType + ",  " + categoryValue);

        Iterable<MediaMetadataCompat> tracks = null;
        // This sample only supports genre and by_search category types.
        if (categoryType.equals(MediaID.MEDIA_ID_MUSICS_BY_GENRE)) {
            tracks = musicProvider.getMusicsByGenre(categoryValue);
        } else if (categoryType.equals(MediaID.MEDIA_ID_MUSICS_BY_SEARCH)) {
            tracks = musicProvider.searchMusicBySongTitle(categoryValue);
        }

        if (tracks == null) {
            Logger.e(TAG, "Unrecognized category type: " + categoryType + " for media " + mediaId);
            return null;
        }
    ...
    public static List<MediaSessionCompat.QueueItem> getPlayingQueueFromSearch(String query,
        ...
        Logger.d(TAG, "Creating playing queue for musics from search: " + query +
                " params=" + queryParams);

        VoiceSearchParams params = new VoiceSearchParams(query, queryParams);

        Logger.d(TAG, "VoiceSearchParams: "+params);
        ...
        return convertToQueue(result, MediaID.MEDIA_ID_MUSICS_BY_SEARCH, query);
    }
    ...
    public static List<MediaSessionCompat.QueueItem> getRandomQueue(MusicProvider musicProvider) {
        ...
        Logger.d(TAG, "getRandomQueue: result.size="+result.size());

        return convertToQueue(result, MediaID.MEDIA_ID_MUSICS_BY_SEARCH, "random");
    }
