package com.gdut.dkmfromcg.commonlib.widget.grouplist;
...
import com.gdut.dkmfromcg.commonlib.util.attr.QMUIAttrHelper;
import com.gdut.dkmfromcg.commonlib.util.display.QMUIDisplayHelper;
import com.gdut.dkmfromcg.commonlib.util.log.Logger;
import com.gdut.dkmfromcg.commonlib.util.trasnform.QMUILangHelper;
import com.gdut.dkmfromcg.commonlib.util.viewhelper.QMUIViewHelper;
 ...
 * 也可以在 xml 中使用 {@link com.gdut.dkmfromcg.commonlib.R.styleable#QMUICommonListItemView_qmui_orientation} 设置。</li>
 * <li>通过 {@link #setAccessoryType(int)} 设置右侧 View 的类型, 可选的类型见 {@link QMUICommonListItemAccessoryType},
 * 也可以在 xml 中使用 {@link com.gdut.dkmfromcg.commonlib.R.styleable #QMUICommonListItemView_qmui_accessory_type} 设置。</li>
 * </ul>
 *
 * @author chantchen
 * @date 2015-1-8
...
public class QMUICommonListItemView extends RelativeLayout {
     ...
     * Item 右侧的 View 的类型
     */
    private @QMUICommonListItemAccessoryType
    int mAccessoryType;
    ...
    private @QMUICommonListItemRedDotPosition
    int mRedDotPosition = REDDOT_POSITION_LEFT;
    ...
    private Drawable qmuiCommonListItemChevronId = null;
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final int initTitleColor = array.getColor(R.styleable.QMUICommonListItemView_qmui_commonList_titleColor,
                getContext().getResources().getColor(R.color.qmui_config_color_gray_1));
        final int initDetailColor = array.getColor(R.styleable.QMUICommonListItemView_qmui_commonList_detailColor,
                getContext().getResources().getColor(R.color.qmui_config_color_gray_5));
        qmuiCommonListItemChevronId = array.getDrawable(R.styleable.QMUICommonListItemView_qmui_common_list_item_chevron);
        if (qmuiCommonListItemChevronId == null) {
            qmuiCommonListItemChevronId = context.getResources().getDrawable(R.drawable.qmui_icon_chevron);
        }
        array.recycle();
    ...
    public void setAccessoryType(@QMUICommonListItemAccessoryType int type) {
        ...
        switch (type) {
            ...
            case ACCESSORY_TYPE_CHEVRON: {
                ...
                tempImageView.setImageDrawable(qmuiCommonListItemChevronId);
...
//            this.requestLayout();
    ...
    public static class Builder {
        ...
        int type = ACCESSORY_TYPE_NONE;
        @QMUICommonListItemRedDotPosition
        int redDotPosition = REDDOT_POSITION_LEFT;
        View accessoryCustomView = null;
        Object tag = null;
        ...
        public Builder setOrientation(@QMUICommonListItemOrientation int orientation) {
            this.orientation = orientation;
            return this;
        }

        public Builder setType(@QMUICommonListItemAccessoryType int type) {
            this.type = type;
            return this;
        }

        public Builder setRedDotPosition(@QMUICommonListItemRedDotPosition int redDotPosition) {
            this.redDotPosition = redDotPosition;
            return this;
        }

        public Builder addAccessoryCustomView(@NonNull View accessoryCustomView) {
            this.accessoryCustomView = accessoryCustomView;
            return this;
        }
        ...
        public final QMUICommonListItemView build() {
            final QMUICommonListItemView itemView = new QMUICommonListItemView(context);
            itemView.setImageDrawable(drawable);
            itemView.setText(text);
            itemView.setDetailText(detailText);
            itemView.setOrientation(orientation);
            if (orientation == HORIZONTAL) {
                int height = QMUIAttrHelper.getAttrDimen(context, R.attr.qmui_list_item_height);
                Logger.d("height: ", height);
                if (height == 0) {
                    height = (int) context.getResources().getDimension(R.dimen.qmui_list_item_height);
                }
                itemView.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, height));
            } else if (orientation == VERTICAL) {
                int heightHigher = QMUIAttrHelper.getAttrDimen(context, R.attr.qmui_list_item_height_higher);
                Logger.d("heightHigher: ", heightHigher);
                if (heightHigher == 0) {
                    heightHigher = (int) context.getResources().getDimension(R.dimen.qmui_list_item_height_higher);
                }
                itemView.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, heightHigher));
            }
            itemView.setAccessoryType(type);
            itemView.setRedDotPosition(redDotPosition);
            if (accessoryCustomView != null) {
                itemView.addAccessoryCustomView(accessoryCustomView);
            }
            if (tag != null) {
                itemView.setTag(tag);
            }
